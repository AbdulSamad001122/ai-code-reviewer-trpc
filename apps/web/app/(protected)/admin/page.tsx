...
export default function AdminPage() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.admin.getUsers.useQuery({ sortBy, sortOrder, search, plan });
  ...
  const planBreakdown = useMemo(() => {
    ...
    stats?.planBreakdown.forEach((p) => {
      map[p.subscriptionPlan] = p._count._all;
    });
    return map;
  }, [stats]);
                ...
                {isLoading ? (
                  ...
                  users?.map((user) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-white/60 max-w-[180px] truncate">
                        {user.email}
                      </td>
                      <td className="px-5 py-4">
                        <span
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <div
                            className="h-1.5 rounded-full bg-[#D7FFA4]/30"
                            style={{ width: "60px" }}
                          >
                            <div
                              className="h-1.5 rounded-full bg-[#D7FFA4]"
                              style={{
                                width: `${Math.min(100, (user.prReviewCount / 50) * 100)}%`,
                              }}
                            />
                          </div>
                      ...
                      <td className="px-5 py-4 text-white/50 tabular-nums">
