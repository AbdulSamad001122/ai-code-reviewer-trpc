...
export function RepoList() {
  ...
  const [hasSyncing, setHasSyncing] = useState(false);
  ...
  const {
    ...
    isPending,
    isError,
  } = trpc.github.getRepos.useInfiniteQuery(
    {},
    {
      ...
      initialPageParam: 1,
      refetchInterval: hasSyncing ? 4000 : false,
    } as any
  );
  ...
  useEffect(() => {
    ...
    const syncing = data.pages.some((page) =>
      page.repos.some(
        (repo: DashboardRepo) => repo.syncStatus === "pending" || repo.syncStatus === "syncing",
      ),
    );
    setHasSyncing(syncing);
  }, [data]);
